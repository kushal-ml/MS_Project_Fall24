...
# Initialize Pinecone
pc = Pinecone(
    api_key="pcsk_4Sf84X_DvHSmK6XtGmgVq27ucduMQPuv4CUhEz4YHdhaT3CuP2gvMGJAXjfXA8qZLmBc8u",
...
def query_medical_knowledge(prompt, options=None):
    # Get embeddings and retrieve relevant chunks as before
    ...
    context = "\n".join([match.metadata.get('text', '').strip() for match in results.matches])
    
    # Create a prompt that includes the context and asks for option selection
    system_prompt = """You are a medical expert. Using the provided medical knowledge, answer the question by selecting the most appropriate option from the given choices. Base your answer solely on the provided context."""
    
    full_prompt = f"""Context from medical knowledge base:
{context}
...
Explanation: [Your reasoning based on the provided context]"""
